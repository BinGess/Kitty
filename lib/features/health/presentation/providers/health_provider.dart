...
final healthSummaryProvider =
    FutureProvider.autoDispose<HealthSummary>((ref) async {
  ...
  if (latestWeight != null) {
    final percent =
        await dao.getWeightChangePercent(cat.id, const Duration(days: 30));
    if (percent != null && percent.abs() > 5) hasWarning = true;
...
final healthTimelineProvider =
  ...
  for (final e in excRecords) {
    ...
    entries.add(HealthTimelineEntry(
      ...
      color: (e.hasBlood || e.hasAnomaly) ? AppColors.error : AppColors.success,
